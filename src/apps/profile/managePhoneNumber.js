...
export const ManagePhoneNumbers = () => {
    ...
    const handlePhoneEntrySubmission = function (e) {
        ...
        let savePhoneNumberButton=$(form).find('button[type="submit"]');

        uiButtons.addLoader(savePhoneNumberButton);

        if (validation === 0) {

            let data={
                dial_code:$(form).find('[name="country_code"]').val(),
                contact_number:$(form).find('[name="user_contact_number"]').val(),
                type:'mobile'
            }

            let phoneNumber = `${data.dial_code}${data.contact_number}`;

            sendOTP(phoneNumber).then(smsResponse => {

                //console.log(smsResponse);
                setCurrentPhoneNumber(data);

                //close the phone entry form 
                setPhoneNumberEntryFormFlag(false);
                ...
                $(form).find('[name="country_code"]').closest('.form-group').append('<div class="required-err">Issue while sending message to phoneNumber. Please try again or check your number.</div>');
                                ...
                                <form id="add-user-contact-info-form" onSubmit={(e) => { handlePhoneEntrySubmission(e) }}>
                                    <div className="form-group">
                                        ...
                                        <div className="text-muted font-weight-bold mb-2">
                                            We will send verification code via message (sms) to confirm your phone number.
                                            Please enter the phone number where you can recieve text messages.
                                        </div>
                                        <div className="row">
                                            <div className="col">
                                                <select name="country_code"
                                                    id="country-code" className="form-control entry-field"
                                                    data-required="1" placeholder="Country code" >
                                                    <option value=""></option>
                                                    {countries.map(c => {
                                            ...
                                            <div className="col">
                                                <input type="text" name="user_contact_number"
